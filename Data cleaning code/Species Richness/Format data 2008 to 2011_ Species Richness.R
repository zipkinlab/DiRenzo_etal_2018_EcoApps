...
# This file formats the species richness data for 2010 to 2011
...
yr <- gsub(pattern= "wet_" , replacement= "" , season_yr)
...
library(plyr)
Frog_data <- Frog_data[Frog_data$Species != "sp.",]
Frog_data$P_A <- ifelse(is.na(Frog_data$Species), 0, 1)

ddply(.data = Frog_data, .variable = "season_yr", .fun = summarize, sr = length(unique(Species)), captures = sum(P_A), 
      length(unique(transect_date)))
...
write.csv(W12, 
          file = 
"/Users/Cici/GitHub/2010to2014ElCope/Data files Nov 2016/SR_WET_2011.csv")
