...
# This file formats the data for 2012 to 2014
...
# Melt the data first into long format
Frog_all1 <- melt(Frog_data, id.vars = c("trans_mtr", "time_date"), measure.vars = c("P_A"))

Frog_all2 <- melt(Frog_data, id.vars = c("trans_mtr", "time_date"), measure.vars = c("Genus_species"))
...
Frog_ALL_1 <- dcast(Frog_all1, trans_mtr ~ time_date, mean)
Frog_ALL_1[Frog_ALL_1 > 0] <- 1

# Sum up the number of frogs found
Frog_ALL_2 <- dcast(Frog_all2, trans_mtr ~ time_date, function(x)length(unique(x)))
...
write.csv(W12, 
          file = 
"/Users/Cici/GitHub/2010to2014ElCope/Data files Nov 2016/SR_WET_2012.csv")
