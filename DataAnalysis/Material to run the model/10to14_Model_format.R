...
NH <- apply(host, c(1, 3), max, na.rm = TRUE)
NH[NH == "-Inf"] <- 0
NH <- colSums(NH)
IH <- apply(Ihost, c(1, 3), max, na.rm = TRUE)
IH[IH == "-Inf"] <- 0
IH <- colSums(IH)
Disease <- rep(c("Uninfected", "Infected"), each = length(NH))
dat <- data.frame(counts = c(NH, IH), Disease = Disease, year = c("2010W", "2011W", "2012W", "2013D", "2013W", "2014D"))
dat$year <- factor(dat$year, levels = c("2010W", "2011W", "2012W", "2013D", "2013W", "2014D"))

library(ggplot2)
ggplot(data = dat, aes(x = year, y = counts, group = Disease, col = Disease)) +  geom_line() + geom_point() +ylab("Total raw amphibian counts across El Copé, Panama") + xlab("Season-Year")+
...
ggsave("Raw_count_plot.pdf", width = 8, height = 5)
