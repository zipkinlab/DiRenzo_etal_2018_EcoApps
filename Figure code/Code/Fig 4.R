...
# This file creates Figure 4 in the paper.
# A 3 panel figure:
...
#--- Correlation tests
NP <- cor.test(c(stand_P), c(stand_N), method = "spearman")
BA <- cor.test(c(difference), c(All2[,-dim(All2)[2]]), method = "spearman")
SA <- cor.test(c(SpecRich), c(abundance), method = "spearman")


# p-value
p <- c(NP$p.value, BA$p.value, SA$p.value)

# correlation coefficent
rho <- c(NP$estimate, BA$estimate, SA$estimate)

stats <- data.frame(p = p, rho = rho)

rownames(stats) <- c("Prev_Abund", "Bd_Abund", "SR_Abund")
