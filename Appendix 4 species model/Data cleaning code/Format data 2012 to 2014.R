...
remove <- c(grep("Puddle", frog$trans), grep("RoadtoRangers", frog$trans), 
            grep("Senderointer", frog$trans))
frog <- frog[-remove,]
...
Pos_neg <- numeric(length(frog3$Zoospore_load_NEW))
...
wet_2012 <- grep(pattern = "[6-8]_.+_2012", time_date)
dry_2013 <- grep(pattern = "[0-5]_.+_2013", time_date)
wet_2013 <- grep(pattern = "[6-8]_.+_2013", time_date)
dry_2014 <- grep(pattern = "[0-5]_.+_2014", time_date)

# Create an empty vector to store season and year
season_yr <- numeric(length(time_date))
...
combin <- data.frame(surv, trans_mtr, trans_date, time_date, season_yr, yr, AM_PM = AM_PM, date = date)
...
Frog_data$P_A2 <- ifelse(is.na(Frog_data$Species), 0, 1)
P_A <- Frog_data$P_A2

## Subset the species you want to keep here

# Replace the NA's with "NOT"
Frog_data$Species <- factor(Frog_data$Species, levels = c("NOT", levels(Frog_data$Species)))

Frog_data$Species[is.na(Frog_data$Species) == TRUE] <- "NOT"

#------ Only need to change this for different species!!!


for(i in 1:nrow(Frog_data)){
  if(!(Frog_data$Species[i] == "albomaculata")){P_A[i] <- 0}
  # prosoblepon
  # cerasinus
  # cruentus
  # albomaculata
...
# Need to adjust for Presence/Absence of animals.
Frog_data$II <- Frog_data$Zoospore_load_NEW * Frog_data$IN2
...
Frog_all <- melt(Frog_data, id.vars = c("trans_mtr", "time_date"), measure.vars = c("II"))
...
write.csv(W12, 
          file = 
            "/Users/Cici/GitHub/4speciesmodel/Data/Salbo/II_DRY_2014.csv")
