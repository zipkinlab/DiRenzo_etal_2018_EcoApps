...
Frog_data$Species2 <- gsub("prosopblepon", "prosoblepon", Frog_data$Species)
Frog_data$Species2 <- gsub("proseblepon", "prosoblepon", Frog_data$Species2)

# Replace the NA's with "NOT"
Frog_data$Species2 <- factor(Frog_data$Species2, levels = c("NOT", levels(Frog_data$Species)))

Frog_data$Species2[is.na(Frog_data$Species2) == TRUE] <- "NOT"

#------ Only need to change this for different species!!!


for(i in 1:nrow(Frog_data)){
  if(!(Frog_data$Species2[i] == "albomaculata")){P_A[i] <- 0}
  # prosoblepon
  # cerasinus
  # cruentus
  # albomaculata
...
Frog_data$II <- Frog_data$Largest.Zoo.Estimate * Frog_data$IN2
...
write.csv(W12, 
          file = 
"/Users/Cici/GitHub/4speciesmodel/Data/Salbo/II_WET_2011.csv")
